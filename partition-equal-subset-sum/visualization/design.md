...
A late-night-debugger aesthetic for walking through an algorithm: deep charcoal panels, a monospaced "trace" voice for code and data, and a warm humanist sans for the explanation that sits beside it. The screen behaves like an instrument panel — a DP table is the instrument, items and capacities are the readings, and three signal colors (amber / cyan / green) tell you exactly what the decision is at every cell. Calm, precise, legible — built to be paused and studied.
...
- DO keep the DP table as the throughline — every step shows a cell being filled
